...
#		  (select ROW_NUMBER() over (order by random(), Id) as R,Yomi,Kaki from LastNames limit @NUM) as L 
#		  inner join  
#		  (select ROW_NUMBER() over (order by random(), Id) as R,Yomi,Kaki,Sex from FirstNames limit @NUM) as F
#		  on L.R=F.R
